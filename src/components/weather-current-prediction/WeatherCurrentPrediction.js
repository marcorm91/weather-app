...
const WeatherCurrentPrediction = ({ hourlyData, diaryData }) => {
  ...
  const currentWind = findPropertyValueByPeriod(currentWindList1, currentHour, ['velocidad']) 
                      || findPropertyValueByPeriod(currentWindList2, currentHour, ['velocidad'])
    ...
    <WeatherCurrentPredictionStyled>
      ...
      <div>
        <ul>
          ...
          <li className='wind__wrapper'><span>{currentWind.velocidad[0]} Km/h</span></li>
        </ul>
      </div>
